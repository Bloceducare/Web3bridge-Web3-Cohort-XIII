...
async function swapTokensForExactTokens(){
    ...
    console.log("Balance in USDC after swap", ethers.formatUnits(usdcBalanceAfter.toString(), 6));
    console.log("Balance in DAI after swap", ethers.formatUnits(daiBalanceAfter.toString(), 18));


}swapTokensForExactTokens().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});