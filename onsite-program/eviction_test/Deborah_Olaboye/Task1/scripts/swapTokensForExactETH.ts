...
async function main() {
  ...
  console.log("User Initial ETH Balance:", ethers.formatUnits(wethBal.toString(), 18));

  const Router = await ethers.getContractAt("IUniSwap", UNIRouter);

  // Amount of USDC to approve 
  const maxUSDCIn = ethers.parseUnits("2000", 6);

  // Approve router to spend USDC
  const approvalUSDC = await USDC.connect(impersonatedSigner).approve(UNIRouter, maxUSDCIn);
  await approvalUSDC.wait();
  console.log("USDC approval done.");

  const path = [USDCAddress, WETH_Address];
  const to = AssetHolder;
  const deadline = Math.floor(Date.now() / 1000) + 60 * 10;

  // Perform the swap
  const ethAmountOut = ethers.parseEther("0.00001");
  ...
  console.log("User Final ETH Balance:", ethers.formatUnits(wethBalAfter.toString(), 18));
}
