...
describe("Lottery Contract", function () {
  ...
  describe("Edge Cases and Error Handling", function () {
    ...
    it("Should prevent joining with 0 ETH", async function () {
      await expect(
        lottery.connect(players[0]).joinLottery({ value: 0 })
      ).to.be.revertedWithCustomError(lottery, "Entry_fee_must_be_exactly_001_ETH");
    });
  });
  ...
  describe("Multiple Rounds", function () {
    ...
    it("Should track correct player count across rounds", async function () {
      // Add 5 players to first round
      for (let i = 0; i < 5; i++) {
        await lottery.connect(players[i]).joinLottery({ value: LOTTERY_PRICE });
      }
      ...
      // Start new round - should be able to add players again
      await expect(
        lottery.connect(players[0]).joinLottery({ value: LOTTERY_PRICE })
      ).to.not.be.reverted;
    });
  ...
  describe("Gas Usage and Performance", function () {
    it("Should efficiently handle participant lookup", async function () {
      // Add several players
      for (let i = 0; i < 8; i++) {
        await lottery.connect(players[i]).joinLottery({ value: LOTTERY_PRICE });
      }
      
      // Try to join again with existing player (should revert efficiently)
      await expect(
        lottery.connect(players[0]).joinLottery({ value: LOTTERY_PRICE })
      ).to.be.revertedWithCustomError(lottery, "You_have_already_joined_this_round");
    });
