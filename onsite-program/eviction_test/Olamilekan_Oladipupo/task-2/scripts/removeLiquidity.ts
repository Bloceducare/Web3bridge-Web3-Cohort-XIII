...
async function main (){
    ...
    const usdcbalance = await USDCContract.balanceOf(assestHolder);
    ...
    await DAIContract.connect(imposterAccount).approve(UNISWAPROUTER, ethers.parseUnits("10000", 18));
    const DaiApprovedAmount = await DAIContract.connect(imposterAccount).allowance(imposterAccount.address, UNISWAPROUTER);
    console.log("DAI Contract approved for Uniswap Router Amount :::::::::::::: ", ethers.formatUnits(DaiApprovedAmount, 18));

    // UNISWAP V2 ADD LIQUIDITY   
    const tx = await uniswap.connect(imposterAccount).addLiquidity(USDCAddress, DAIAddress, USDCAMOUNT, DAIAMOUNT, USDCAMOUNTMIN, DAIAMOUNTMIN, imposterAccount.address,DEADLINE);
    const receipt = await tx.wait();
    console.log("Transaction status ::::::::::::::::", receipt.status ===1 ? "Success" : "Failed");

    const usdcbalance1 = await USDCContract.balanceOf(assestHolder);
    const daiBalance1 = await DAIContract.balanceOf(assestHolder);
    console.log("Imposter account balance after from usdc:::::::::::::::", ethers.formatUnits(usdcbalance1.toString(), 6));
    console.log("Imposter account balance after from dai:::::::::::::::", ethers.formatUnits(daiBalance1.toString(), 18));
