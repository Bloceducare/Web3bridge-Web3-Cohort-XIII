...
async function main() {
  ...
  const amountIn = ethers.parseUnits("5", daiDecs); // 5 DAI
  await approveIfNeeded(dai, me, UNISWAP_V2_ROUTER, amountIn);

  const amountsOut: bigint[] = await router.getAmountsOut(amountIn, path);
  const minOut = subBps(amountsOut[1], 100n); // 1% slippage

  const deadline = BigInt(Math.floor(Date.now() / 1000) + 60 * 10);

  const beforeIn = await dai.balanceOf(me);
  const beforeOut = await usdc.balanceOf(me);

  console.log("Signer:", me);
  console.log(
    `Selling ${ethers.formatUnits(amountIn, daiDecs)} ${daiSym} for at least ${ethers.formatUnits(minOut, usdcDecs)} ${usdcSym}`
  );
  ...
  const afterIn = await dai.balanceOf(me);
  const afterOut = await usdc.balanceOf(me);

  console.log(
    `Delta ${daiSym}: -${ethers.formatUnits(beforeIn - afterIn, daiDecs)}`
  );
  console.log(
    `Delta ${usdcSym}: +${ethers.formatUnits(afterOut - beforeOut, usdcDecs)}`
  );
}
