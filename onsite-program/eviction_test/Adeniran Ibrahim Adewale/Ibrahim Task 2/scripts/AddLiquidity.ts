...
async function main() {
  ...
  const ApproveUsdc = await Usdc.connect(impersonatedSigner).approve(
    UNIROUTER,
    USDCAmount
  );
...
//   const AllowanceUsdc = await Usdc.allowance(assetHolder, UNIROUTER);
  ...
  console.log(
    "User Balance in Usdc after adding liquidity",
    ethers.formatUnits(usdcBalanceAfter.toString(), 6)
  );
  console.log(
    "User Balance in DAI after adding liquidity",
    ethers.formatUnits(daiBalanceAfter.toString(), 18)
  );
}
main().catch((error) => {
  console.error(error);
