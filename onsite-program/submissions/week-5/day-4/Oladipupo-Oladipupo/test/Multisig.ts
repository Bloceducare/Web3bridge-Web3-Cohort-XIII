...
//       const txId = await multiSig.connect(signer1).proposeTransaction(signer2.address, 1000);
//       expect(await txId).to.be.a("bigint");
//     });
...
//     it("should allow a different signer to sign a transaction", async function () {
//       const { multiSig, signer1, signer2, signer3 } = await loadFixture(deployMultiSig);
//       const txId = await multiSig.connect(signer1).proposeTransaction(signer3.address, 1000);

//       await expect(multiSig.connect(signer2).signTransaction(0))
//         .to.emit(multiSig, "TransactionSigned") // if you later add events
...
//       await expect(
//         multiSig.connect(signer2).signTransaction(0)
...
//       await multiSig.connect(signer3).signTransaction(0); // should trigger execution
