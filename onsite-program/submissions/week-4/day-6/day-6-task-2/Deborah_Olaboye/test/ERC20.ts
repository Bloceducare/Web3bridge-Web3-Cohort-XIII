...
describe("ERC20", function () {
  ...
  it("Should transfer tokens between accounts", async function () {
    const { erc20, owner, address1 } = await loadFixture(deployERC20);
    await erc20.transfer(address1.address, 100);
    expect(await erc20.balanceOf(address1.address)).to.equal(100);
    expect(await erc20.balanceOf(owner.address)).to.equal(900);
  });

  it("Should approve allowance for another account", async function () {
    const { erc20, owner, address1 } = await loadFixture(deployERC20);
    await erc20.approve(address1.address, 200);
    expect(await erc20.allowance(owner.address, address1.address)).to.equal(200);
  });

  it("Should transfer tokens using transferFrom", async function () {
    const { erc20, owner, address1, address2 } = await loadFixture(deployERC20);
    await erc20.approve(address1.address, 150);
    await erc20.connect(address1).transferFrom(owner.address, address2.address, 150);
    expect(await erc20.balanceOf(address2.address)).to.equal(150);
    expect(await erc20.balanceOf(owner.address)).to.equal(850);
    expect(await erc20.allowance(owner.address, address1.address)).to.equal(0);
  });
  ...
  it("Should not allow transfer of more than balance", async function () {
    const { erc20, owner, address1 } = await loadFixture(deployERC20);
    await expect(erc20.connect(address1).transfer(owner.address, 1)).to.be.revertedWith(
  ...
  it("Should not allow transferFrom of more than allowance", async function () {
    const { erc20, owner, address1, address2 } = await loadFixture(deployERC20);
    await erc20.approve(address1.address, 100);
    await expect(
      erc20.connect(address1).transferFrom(owner.address, address2.address, 101)
