...
describe("TeacherManagement", function () {
  async function deployTeacherManagementFixture() {
    ...
    const TeacherManagement = await hre.ethers.getContractFactory(
      "TeacherManagement"
    );
  ...
  describe("Deployment", function () {
    it("Should deploy without errors", async function () {
      const { teacherManagement } = await loadFixture(
        deployTeacherManagementFixture
      );
  ...
  describe("User Registration", function () {
    it("Should register a new user successfully", async function () {
      const { teacherManagement, user1 } = await loadFixture(
        deployTeacherManagementFixture
      );
      const name = "John Doe";
      ...
      await expect(
        teacherManagement
          .connect(user1)
          .registerUser(name, userType, agreedSalary)
      )
    ...
    it("Should revert if name is empty", async function () {
      ...
      await expect(
        teacherManagement
          .connect(user1)
          .registerUser("", 0, hre.ethers.parseEther("1"))
    ...
    it("Should revert if user is already registered", async function () {
      ...
      //   console.log('Begining', {teacherManagement, user1})

      const name = "John Doe";
      const userType = 0;
      const agreedSalary = hre.ethers.parseEther("1");

      const regUser1 = await teacherManagement
        .connect(user1)
        .registerUser(name, userType, agreedSalary);

      // console.log('registered user 1', regUser1)

      const regUser2 =  expect(
        teacherManagement
          .connect(user1)
          .registerUser(name, userType, agreedSalary)
      );

      //   console.log('registered user 2', regUser2)

      regUser2.to.be
        .revertedWithCustomError(teacherManagement, "UserAlreadyRegistered")
        .withArgs(user1.address);
    });
  });
  ...
  describe("User Update", function () {
    it("Should update user details successfully", async function () {
      ...
      await teacherManagement
        ...
        .registerUser(name, userType, agreedSalary);
      await expect(
        teacherManagement.connect(user1).updateUser(0, newName, newSalary)
      )
    ...
    it("Should revert if user does not exist", async function () {
      ...
      await expect(
        teacherManagement
          .connect(user1)
          .updateUser(999, "Jane Doe", hre.ethers.parseEther("1"))
      )
        .to.be.revertedWithCustomError(teacherManagement, "UserNotFound")
        .withArgs(999);
    });

    it("Should revert if sender is not the user", async function () {
      const { teacherManagement, user1, user2 } = await loadFixture(
        deployTeacherManagementFixture
      );
      await teacherManagement
        .connect(user1)
        .registerUser("John Doe", 0, hre.ethers.parseEther("1"));
      await expect(
        teacherManagement
          .connect(user2)
          .updateUser(0, "Jane Doe", hre.ethers.parseEther("1"))
      )
        .to.be.revertedWithCustomError(teacherManagement, "UserNotFound")
        .withArgs(0);
    });
  ...
  describe("User Termination", function () {
    ...
    it("Should revert if user does not exist", async function () {
      ...
      await expect(teacherManagement.connect(user1).terminateUser(999))
        .to.be.revertedWithCustomError(teacherManagement, "UserNotFound")
        .withArgs(999);
    });
  });
  ...
  describe("Salary Disbursement", function () {
    it("Should disburse salary successfully", async function () {
      ...
      await teacherManagement
        .connect(user1)
        .registerUser("John Doe", 0, agreedSalary);
      const tx = teacherManagement
        .connect(user1)
        .disburseSalary(0, amount, { value: amount });
      await expect(tx)
        .to.emit(teacherManagement, "SalaryDisbursed")
        .withArgs(user1.address, 0, amount);
      await expect(tx).to.changeEtherBalances(
        [user1, teacherManagement],
        [amount, -amount]
      );
    ...
    it("Should revert if insufficient balance sent", async function () {
      ...
      await expect(
        teacherManagement
          .connect(user1)
          .disburseSalary(0, amount, { value: hre.ethers.parseEther("0.2") })
      )
    ...
    it("Should revert if user is not employed", async function () {
      ...
      await expect(
        ...
        .to.be.revertedWithCustomError(teacherManagement, "UserNotEmployed")
        .withArgs(0);
    });
  });
  ...
  describe("User Queries", function () {
    ...
    it("Should return users by type", async function () {
      ...
      await teacherManagement
        .connect(user2)
        .registerUser("Jane Doe", 0, hre.ethers.parseEther("1"));
    ...
    it("Should return all users", async function () {
      ...
      await teacherManagement
        .connect(user2)
        .registerUser("Jane Doe", 1, hre.ethers.parseEther("1"));
