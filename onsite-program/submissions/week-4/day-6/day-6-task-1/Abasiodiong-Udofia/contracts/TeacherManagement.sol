...
contract TeacherManagement is ITeacherManagement {
    ...
    function registerUser(
        ...
        if (addressToId[msg.sender] != 0)
            revert Errors.UserAlreadyRegistered(msg.sender); // Prevent re-registration
    ...
    function disburseSalary(
        ...
        if (_amount > users[_id].agreedSalary)
            revert Errors.SalaryExceedsAgreedAmount(
                _amount,
                users[_id].agreedSalary
            );
        if (msg.value < _amount)
            revert Errors.InsufficientBalance(_amount, msg.value);
    ...
    function getUsersByType(
        ...
        uint256[] memory activeUsers = new uint256[](
            usersByType[_userType].length
        );
        uint256 count = 0;
