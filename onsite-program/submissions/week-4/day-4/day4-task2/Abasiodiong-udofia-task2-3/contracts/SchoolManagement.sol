...
contract SchoolManagement {
    ...
    function registerStudent(string memory _name, uint256 _age) public {
        require(
            bytes(schools[msg.sender].name).length > 0,
            "School not created"
        );
        ...
        school.students[studentId] = Student(
            studentId,
            _name,
            _age,
            Status.ACTIVE,
    ...
    function updateStudent(
        ...
        require(
            schools[msg.sender].students[_id].exists,
    ...
    function getStudent(
        uint256 _id
    ) public view returns (uint256, string memory, uint256, Status, bool) {
        require(
            bytes(schools[msg.sender].name).length > 0,
            "School not created"
        );
