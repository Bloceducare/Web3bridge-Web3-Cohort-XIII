...
contract StudentMgt{
    ...
    function register(string memory _firstName, string memory _lastName, string memory _department, uint _studentId, uint8 _age) external{
        students.push(Student(_firstName, _lastName, _department, _studentId, _age, Status.ACTIVE));
        nextStudentId++;
    }
    ...
    function getStudentById(uint _studentId) external view returns (Student memory student){
