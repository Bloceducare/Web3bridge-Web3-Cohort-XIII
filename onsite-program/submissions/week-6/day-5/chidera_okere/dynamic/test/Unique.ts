...
describe('ClockNFT', function () {
  ...
  describe('Minting', function () {
    ...
    it('Should allow different accounts to mint', async function () {
      const { clockNFT, owner, otherAccount } = await loadFixture(deployClockNFTFixture)
  ...
  describe('TokenURI', function () {
    ...
    it('Should revert for non-existent token', async function () {
      ...
      await expect(clockNFT.tokenURI(0)).to.be.revertedWith('NFT does not exist')
    })
    ...
    it('Should contain correct metadata structure', async function () {
      ...
      expect(metadata.external_url).to.include('https://your-website.com/clock/0')
    })
    ...
    it('Should have correct attributes in metadata', async function () {
      ...
      const typeAttr = attributes.find((attr: any) => attr.trait_type === 'Type')
      expect(typeAttr.value).to.equal('Dynamic Clock')

      const timestampAttr = attributes.find((attr: any) => attr.trait_type === 'Timestamp')
      expect(parseInt(timestampAttr.value)).to.be.greaterThan(0)

      const timeAttr = attributes.find((attr: any) => attr.trait_type === 'Time')
      expect(timeAttr.value).to.include('UTC')

      const blockAttr = attributes.find((attr: any) => attr.trait_type === 'Block Number')
    ...
    it('Should format time correctly', async function () {
      ...
      const timeAttr = metadata.attributes.find((attr: any) => attr.trait_type === 'Time')
    ...
    it('Should have different tokenURIs for different token IDs', async function () {
      ...
      const metadata0 = JSON.parse(Buffer.from(base64Data0, 'base64').toString('utf-8'))

      const base64Data1 = tokenURI1.replace('data:application/json;base64,', '')
      const metadata1 = JSON.parse(Buffer.from(base64Data1, 'base64').toString('utf-8'))
  ...
  describe('ERC721 Standard Compliance', function () {
    ...
    it('Should allow approval and transferFrom', async function () {
      ...
      await clockNFT.connect(otherAccount).transferFrom(owner.address, otherAccount.address, 0)
    ...
    it('Should allow setting approval for all', async function () {
      ...
      expect(await clockNFT.isApprovedForAll(owner.address, otherAccount.address)).to.be.true
