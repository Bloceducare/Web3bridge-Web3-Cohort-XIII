...
contract ERC7432 is IERC7432 {
    ...
    mapping(address => mapping(uint256 => mapping(address => bytes32))) recipientRoles;
    ...
    function grantRole(Role calldata _role) external {
        ...
        require(recipientRoles[tokenAddress][tokenId][_role.recipient] == bytes32(0), ROLE_ALREADY_GRANTED_FOR_TOKEN()); 

        recipientRoles[tokenAddress][tokenId][_role.recipient] = _role.roleId;
    ...
    function recipientOf(address _tokenAddress, uint256 _tokenId, bytes32 _roleId) external view returns (address) {
        if (roleExpiration[_tokenAddress][_tokenId][_roleId] > block.timestamp) {
            return recipientRoles[_tokenAddress][_tokenId][_roleId];
        }
        return address(0);
    }
}