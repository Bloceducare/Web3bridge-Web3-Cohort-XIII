...
export function StakingPosition() {
  ...
  const handleEmergencyWithdraw = async () => {
    ...
    try {
      ...
      toast(
        ...
        //   description:
              "Emergency withdrawal has been submitted",
      )
    } catch (error) {
      toast(
        //   description:
  ...
  const stakeAmount = stakeData ? formatEther(stakeData[0]) : 0
  const stakeTimestamp = stakeData ? Number(stakeData[1]) : 0
  const rewardAmount = pendingRewards ? formatEther(pendingRewards) : 0
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                  Staked since: {new Date(stakeTimestamp * 1000).toLocaleDateString()}
                </span>
              </div>
