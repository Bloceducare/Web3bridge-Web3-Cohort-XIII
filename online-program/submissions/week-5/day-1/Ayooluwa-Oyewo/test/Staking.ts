...
describe("Staking", function () {
  ...
  async function deployOneYearLockFixture() {
    ...
    const staking = await Staking.deploy(tokenA.target, tokenB.target, lockPeriod);
    await tokenB.setMinter(staking.target, true); // Set staking contract as minter

    return { staking, tokenA, Staking, tokenB, lockPeriod, owner, otherAccount };
  }

  describe("Deployment", function () {
    it("Should deploy staking token", async function () {
      const { staking, tokenA, tokenB, lockPeriod } = await loadFixture(deployOneYearLockFixture);
    ...
    it("Should not deploy with zero address", async function () {
      const { tokenA, tokenB, Staking } = await loadFixture(deployOneYearLockFixture);

      await expect(Staking.deploy(tokenA.target, tokenB.target, 0)).to.be.revertedWithCustomError(Staking, "Staking_NoLockPeriodSet");
    });
    it("Should not deploy with zero address for tokenA", async function () {
      ...
      await expect(Staking.deploy(hre.ethers.ZeroAddress, tokenB.target, 30 * 24 * 60 * 60)).to.be.revertedWithCustomError(Staking, "Staking_StakingTokenAddressNotSet");
    });
    it("Should not deploy with zero address for tokenB", async function () {
      ...
      await expect(Staking.deploy(tokenA.target, hre.ethers.ZeroAddress, 30 * 24 * 60 * 60)).to.be.revertedWithCustomError(Staking, "Staking_RewardTokenAddressNotSet");
    });
  ...
  describe("Stake Function", function () {
     it("Should not allow staking with zero amount", async function () {
        const { staking, otherAccount } = await loadFixture(deployOneYearLockFixture);
      await expect(staking.connect(otherAccount).stake(0)).to.be.revertedWithCustomError(staking, "Staking_ZeroAmount");
    });

    it("Should not allow staking more than balance", async function () {
       const { staking, tokenA, otherAccount } = await loadFixture(deployOneYearLockFixture);

    // otherAccount has only 1000 tokenA — try to stake more than that
    await tokenA.connect(otherAccount).approve(staking.target, hre.ethers.parseEther("2000"));
   ...
   it("Should not allow staking if already staked", async function () {
    ...
    await tokenA.connect(otherAccount).approve(staking.target, hre.ethers.parseEther("100"));
    ...
    it("Should allow staking and emit event", async function () {
      ...
      await expect(staking.connect(otherAccount).stake(hre.ethers.parseEther("100")))
  ...
  describe("Unstake Function", function () {
     it("Should not allow unstaking with zero amount", async function () {
        const { staking, otherAccount } = await loadFixture(deployOneYearLockFixture);
      await expect(staking.connect(otherAccount).unstake(0)).to.be.revertedWithCustomError(staking, "Staking_ZeroAmount");
    });
    it("Should not allow unstaking if not staked", async function () {
      const { staking, otherAccount } = await loadFixture(deployOneYearLockFixture);
      
      await expect(staking.connect(otherAccount).unstake(hre.ethers.parseEther("100"))).to.be.revertedWithCustomError(staking, "Staking_NotStaked");
    });
    it("Should not allow unstaking more than staked amount", async function () {
      ...
      await expect(staking.connect(otherAccount).unstake(hre.ethers.parseEther("200"))).to.be.revertedWithCustomError(staking, "Staking_InsufficientBalance");
    });
    it("Should not allow unstaking before lock period", async function () {
      const { staking, tokenA, otherAccount, lockPeriod } = await loadFixture(deployOneYearLockFixture);

      await tokenA.connect(otherAccount).approve(staking.target, hre.ethers.parseEther("100"));
      await staking.connect(otherAccount).stake(hre.ethers.parseEther("100"));

      // Try to unstake before lock period
      await expect(staking.connect(otherAccount).unstake(hre.ethers.parseEther("100"))).to.be.revertedWithCustomError(staking, "Staking_LockPeriodNotOver");
    });
    it("Should allow unstaking after lock period and emit event", async function () {
      ...
      await expect(staking.connect(otherAccount).unstake(hre.ethers.parseEther("100")))
